...
import StudentModal2 from './StudentModal2';
import StudentModal from './StudentModal';
const Table1 = () => {
  const [darkMode, setDarkMode] = useState(false);
  const [stuModal1, setStuModal1] = useState(false);
  const [stuModal2, setStuModal2] = useState(false);
  const [student, setStudent] = useState([]);
  const [studentID, setStudentID] = useState('')


  const getID = (id) => { setStudentID(id) }
  const delStudent = async (id) => {
    let res = await axios.delete(`${config.url}/admin/students/${id}`, {
      headers: {
  ...
  const chModal1 = () => {
    setStuModal1(!stuModal1);
  };
  // const profile1 = () => {
  //   setProfile(!profile);
  // };
  const chModal2 = () => {
    setStuModal2(!stuModal2);
  };
    ...
    <>
      <div className={`App ${darkMode ? 'dark' : 'light'}`}>
        <main className="bg-white dark:bg-gray-900 h-screen pt-10">
                    ...
                    return (
                      <tr key={item._id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td className="px-6 py-4 text-right w-32">
                          ...
                          <button onClick={() => { delStudent(item._id) }} className="ml-3 font-medium text-blue-600 dark:text-blue-500 hover:underline"><i
          ...
          {stuModal2 && <StudentModal2 fn={chModal2} />}
          {stuModal1 && <StudentModal fn={chModal1} studentID={studentID} />}
          {/* {profile && <Profile />} */}
          {/* <!-- Modal post --> */}
          ...
          {stuModal1 && <div onClick={chModal1} className="bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40"></div>}
          {stuModal2 && <div onClick={chModal2} className="bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40"></div>}
